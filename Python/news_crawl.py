...
def search_for_baidu_news_posts():
    last_time = session.query(Job).filter(Job.info_source_id==BAIDU_NEWS_INFO_SOURCE_ID).order_by(Job.id.desc()).first().previous_executed    
        ...
        while(not finished):
            ...
            for news_table in news_tables:
                ...
                add_news_to_session(url, source_name, title, content,
                                    BAIDU_NEWS_INFO_SOURCE_ID, created_at, keyword.id)
...
def search_for_google_news_posts():
    last_time = session.query(Job).filter(Job.info_source_id==GOOGLE_NEWS_INFO_SOURCE_ID).order_by(Job.id.desc()).first().previous_executed    
    ...
    sql_job = Job()
    sql_job.previous_executed = datetime.now()
    sql_job.info_source_id = GOOGLE_NEWS_INFO_SOURCE_ID
    ...
    for keyword in KEYWORDS :
        ...
        while(not finished):
            ...
            for news_table in news_tables:
                ...
                add_news_to_session(url, source_name, title, content,
                                    GOOGLE_NEWS_INFO_SOURCE_ID, created_at, keyword.id)
...
def main():
    try:
        search_for_baidu_news_posts()
        search_for_google_news_posts()
    except Exception, e:
