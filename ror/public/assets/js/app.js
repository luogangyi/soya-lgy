var App = function () {
        ...
        $('.sidebar-search input').keypress(function (e) {
            if (e.which == 13) {
                if(jQuery(this).val().length>0){
                    window.location.href = "/search/index?query="+jQuery(this).val();
        ...
        $('.sidebar-search .submit').click(function () {
            if ($('.page-container').hasClass("sidebar-closed")) {
                if ($('.sidebar-search').hasClass('open') == false) {
                    ...
                    if($('.sidebar-search input').val().length>0){
                        window.location.href = "/search/index?query="+$('.sidebar-search input').val();
                        return false; //<---- Add this line
                    }else{
                        alert("请输入搜索内容！");
                        return false;
    ...
    var handleDashboardCharts = function () {
        ...
        var data3=[{'03-25<br>16:00' : {y:1}},{'03-25<br>17:00' : {y:5}},{'03-25<br>18:00' : {y:2}},{'03-25 19:00' : {y:6}},{'03-25 20:00' : {y:6,marker: {symbol: 'url(../static/images/sun.png)'}}},{'03-25 21:00' : {y:5}},{'03-25 22:00' : {y:4}},{'03-25 23:00' : {y:5}},{'03-26 00:00' : {y:3,marker: {symbol: 'url(../static/images/sun.png)'}}},{'03-26 01:00' : {y:1}},{'03-26 02:00' : {y:0}},{'03-26 03:00' : {y:0}},{'03-26 04:00' : {y:0,marker: {symbol: 'url(../static/images/sun.png)'}}},{'03-26 05:00' : {y:1}},{'03-26 06:00' : {y:1}},{'03-26 07:00' : {y:0}},{'03-26 08:00' : {y:2,marker: {symbol: 'url(../static/images/sun.png)'}}},{'03-26 09:00' : {y:8}},{'03-26 10:00' : {y:3}},{'03-26 11:00' : {y:3}},{'03-26 12:00' : {y:3,marker: {symbol: 'url(../static/images/sun.png)'}}},{'03-26 13:00' : {y:2}},{'03-26 14:00' : {y:5}},{'03-26 15:00' : {y:1}}];
		
		
        var data1 = [ [1, randValue() - 5],
            [2, randValue() - 5],
            [3, randValue() - 5],
            [4, 6 + randValue()],
            [5, 5 + randValue()],
		...
		if (data3.length > 0) {
			...
			$.each(data3, function(index, elm) {
				for ( var i in elm) {
					alldata[0].data.push(data1[index][1]);
					alldata[1].data.push(data2[index][1]);
					if((index+1)%2==0)
						cartograms.xaxis.ticks.push([index,i]);
        ...
        $("#site_statistics").bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                if (previousPoint != item.dataIndex) {
                    ...
                    $("#tooltip").remove();
                    var x = data2[item.dataIndex][1],
                        y = data1[item.dataIndex][1];
...
//            if (e.which == 13) {
//                window.location.href = "../dosa/overview";
//                return false;
//            }
//        });
    ...
    var handleEditableTables = function () {
        ...
        function editRow(oTable, nRow) {
            ...
            jqTds[0].innerHTML = '<select class="m-wrap span12" data-placeholder="Choose a Category" tabindex="1">'+
                                    '<option value="关键词分类">关键词分类</option>'+
                                    '</select>';
        ...
        function saveRow(oTable, nRow) {
            ...
            oTable.fnUpdate('<form action="#" class="form-horizontal"><div class="control-group"><div class="controls"><input id="tags_1" type="text" class="m-wra newtags" value="" /></div></div></form>', nRow, 2, false);
            oTable.fnUpdate('<a class="delete" href="">删除</a>', nRow, 3, false);
            oTable.fnDraw();
            $('.newtags').tagsInput({
            	width: 'auto',
            	'onAddTag': function () {$(this).removeClass('newtags');
            	},
            });
...
*/
        ...
        $('#sample_editable_1_new').click(function (e) {
            e.preventDefault();
            var aiNew = oTable.fnAddData(['','', '','<a class="edit" href="">保存</a> | <a class="cancel" data-mode="new" href="">取消</a>']);
    ...
    var handleMyDateTimePickers = function(){
		...
		dStartTime.val(yesterday.to_s());
		dStartHours.val(0);
		dEndTime.val(today.to_s());
		dEndHours.val(nowHours > 23 ? 23 : nowHours);

		dStartTime.datepicker({
					dateFormat : 'yy-mm-dd',
					maxDate : dEndTime.val(),
					beforeShow : function(input, inst) {
						$(inst.dpDiv).removeClass().addClass(
								'calendar input-calendar');
					},
					onSelect : function(dateText, inst) {
						dEndTime.datepicker('option', 'minDate', dateText);
					}
				});

		dEndTime.datepicker({
					dateFormat : 'yy-mm-dd',
					minDate : dStartTime.val(),
					maxDate : new Date(),
					beforeShow : function(input, inst) {
						$(inst.dpDiv).removeClass().addClass(
								'calendar input-calendar');
					...
					onSelect : function(dateText, inst) {
						dStartTime.datepicker('option', 'maxDate', dateText);
						var time = dEndTime.datepicker("getDate"), today = new Date(), nowHours = today
								.getHours() + 1;
						if (time.to_s() < today.to_s()) {
							dEndHours.val(23);
						} else {
							dEndHours.val(nowHours > 23 ? 23 : nowHours);
						}
    ...
    var handletags = function(){
		...
		$(".icon_edit").live('click',
				function() {
					...
					$(this).parent().parent().parent().parent().parent().find(".edit_tags")
							.slideDown('fast');
				});
		$("body").live('click',
				function(e) {
			...
			if ($(this).hasClass("emotion_sad")) {
				$(this).addClass("emotion_good");
				$(this).removeClass("emotion_sad");
				$(this).removeClass("emotion_middle");
				$(this).html("正面消息");
			} else if ($(this).hasClass("emotion_good")) {
				$(this).addClass("emotion_middle");
				$(this).removeClass("emotion_sad");
				$(this).removeClass("emotion_good");
				$(this).html("中性消息");
			} else if ($(this).hasClass("emotion_middle")) {
				$(this).addClass("emotion_sad");
				$(this).removeClass("emotion_good");
				$(this).removeClass("emotion_middle");
				$(this).html("负面消息");
			}
		...
		$(".stars li").live('click',
				function() {
					//$("#alert").show();
					//var theid=$(this).parent().parent().parent().parent().parent().parent().attr('id');
					$(this).parent().removeClass();
					$(this).parent().addClass("stars stars"+($(this).index() + 1));
					
					/*
$.get('/dosa/inbox/modifyweibourgency?weibo_id='+theid+'&u='+($(this).index() + 1)).done(function() {
						$("#alert").hide();
					})
